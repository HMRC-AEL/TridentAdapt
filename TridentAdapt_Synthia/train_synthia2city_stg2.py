...
source_loader = torch_data.DataLoader(synthia_set, batch_size=batch_size, shuffle=True, num_workers=1, pin_memory=True)
...
city_set = CityLoader(args.city_data_path, args.data_list_path_city_img, args.data_list_path_city_lbl,
                      ...
                      mean=IMG_MEAN, set='train', use_pseudo = True)
...
dec_s = ImgDecoder(bottleneck_channel).cuda()
dec_t = ImgDecoder(bottleneck_channel).cuda()
...
for i_iter in range(num_steps):
    ...
    if i_iter < 1:
        rec_s = dec_s(shared_feature_s_shallow.detach())
        rec_t = dec_t(shared_feature_t_shallow.detach())
        rec_t2s = dec_s(shared_feature_t_shallow.detach())
        rec_s2t = dec_t(shared_feature_s_shallow.detach())
        ...
        rec_s = dec_s(shared_feature_s_shallow)
        rec_t = dec_t(shared_feature_t_shallow)
        rec_t2s = dec_s(shared_feature_t_shallow)
        rec_s2t = dec_t(shared_feature_s_shallow)
    ...
    if i_iter % 1 == 0:
        ...
        loss_d_s2t = 0
        for it, (prob_dis_s2t_real1, prob_dis_s2t_fake1) in enumerate(
                zip(prob_dis_s2t_real1_list, prob_dis_s2t_fake1_list)):
    ...
    if i_iter % 1 == 0:
        ...
        loss_d_t2s = 0
        for it, (prob_dis_t2s_real1, prob_dis_t2s_fake1) in enumerate(
                zip(prob_dis_t2s_real1_list, prob_dis_t2s_fake1_list)):
    ...
    # ==== Image self-reconstruction loss  & semantic consistency loss ====
    ...
    loss_s_sg1 = sg_loss(t_pred1_up, tlabelv_pseudo)
    loss_s_sg2 = sg_loss(t_pred2_up, tlabelv_pseudo)

    loss_sim_sg += 0.75*(lambda_seg * loss_s_sg1 + loss_s_sg2)
    ...
    if i_iter % 500 == 0:
        imgs_s = torch.cat(((sdatav[:, [2, 1, 0], :, :].cpu() + 1) / 2, (rec_s[:, [2, 1, 0], :, :].cpu() + 1) / 2,
        ...
        imgs_t = torch.cat(((tdatav[:, [2, 1, 0], :, :].cpu() + 1) / 2, (rec_t[:, [2, 1, 0], :, :].cpu() + 1) / 2,
                            (rec_t2s[:, [2, 1, 0], :, :].cpu() + 1) / 2,
                            Variable(torch.Tensor((map_t.transpose((0, 3, 1, 2))))),
                            Variable(torch.Tensor((gt_t_pseudo.transpose((0, 3, 1, 2))))),
