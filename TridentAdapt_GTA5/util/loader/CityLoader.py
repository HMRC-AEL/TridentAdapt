...
class CityLoader(data.Dataset):
	def __init__(self, root, img_list_path, lbl_list_path, max_iters=None, crop_size=None, mean=(128, 128, 128), transform=None, set='val', return_name = False, use_pseudo = False):
		self.n_classes = 19
		self.root = root
	...
	def __getitem__(self, index):
		...
		if not self.use_pseudo:
			...
			label = np.asarray(label, np.long)
			pseudo_label = np.asarray(pseudo_label, np.long)
			...
			pseudo_label_copy = 255 * np.ones(pseudo_label.shape, dtype=np.long)
